...
- `Dane testowe` — patrz `.claude/skills/qa-case/columns.md` sekcja Dane testowe (dla wyszukiwarek: lista przecinkami, pierwsza wartość = użyta w Krokach)
- `Kroki` = numerowane, imperatywne, deterministyczne, bez weryfikacji. **URL-e zawsze pełne** (`https://...`), nie relatywne ścieżki. Format zależy od typu pliku — patrz krok 6.
