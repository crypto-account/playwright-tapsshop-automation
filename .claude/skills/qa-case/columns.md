...
- **English jargon**: nie „sanitized", „end-to-end", „empty state", „XSS", „keyboard-only", „focus visible", „target size", „screen reader", „assistive tech (AT)", „debounce", „viewport", „scrollWidth/clientWidth", „responsive breakpoint", „case-insensitive", „case-sensitive", „trim", „truncate", „bundle", „edge case". Używaj: „nie zostaje wykonany", „pełny przepływ", „brak wyników", „złośliwy kod", „obsługa z klawiatury", „widoczny fokus", „rozmiar celu", „czytnik ekranu", „technologie wspomagające", „opóźnienie odświeżenia listy" LUB „poczekaj aż lista sama się odświeży", „rozmiar okna przeglądarki", „poziomy pasek przewijania", „rozmiar okna", „wielkość liter ignorowana" LUB „niezależnie od wielkości liter", „wielkość liter rozróżniana", „przycięcie spacji na krańcach", „skrócenie", „grupowanie / w jednym przypadku", „przypadek brzegowy". Ponadto: **nie dodawaj uwag out-of-scope** do Oczekiwanego rezultatu — jeśli case nie testuje danej cechy (np. case-insensitivity), NIE wspominaj o niej w expected (uwaga zmyla). Techniczne akronimy tolerowane tylko gdy to nazwa własna funkcji produktu (IC, WCAG, PZZJ, ARIA, NIP) LUB gdy komunikują logikę filtrów precyzyjniej niż PL opis: **AND / OR w nawiasie** dozwolone dla cases łączących filtry, np. „…pokazuje tylko wspólne wyniki (logika AND)" / „…pokazuje sumę wyników (logika OR)"
...
**Explicit label** — wartość deterministyczna (używana w Krokach) oddzielona od alternatyw do testowania manualnego:
